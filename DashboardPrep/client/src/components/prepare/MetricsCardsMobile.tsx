...
export default function MetricsCardsMobile({ state, esResult, maskBuffer }: MetricsCardsMobileProps) {
  ...
  return (
    <div className="flex justify-center gap-2">
      {metricCards.map((card, index) => (
        <div 
          key={index} 
          className={`glass-card-mobile flex flex-col items-center justify-center px-2 py-1 rounded-lg border ${card.color} min-w-[60px]`}
        >
          <div className="text-xs font-bold leading-tight">
            {card.value}
            {card.subValue && (
              <span className="text-xs opacity-70 ml-1">{card.subValue}</span>
            )}
          </div>
